...
class TransactionDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // Obtenemos la lista de transacciones del provider
    ...
    return transactionListAsync.when(
      ...
      data: (transactions) {
        // Buscamos la transacción por ID
                              ...
                              children: [
                                ...
                                if (transaction.recurrence != 'Única vez')
                                  Positioned(
                                    ...
                                    child: Tooltip(
                                      message:
                                          'Transacción recurrente: ${transaction.recurrence}',
                                      child: Icon(
    ...
    return Container(
      ...
      child: IconButton(
        onPressed: () async {
          ...
          if (hasPermission) {
            // 2. Buscamos la transacción actual en la lista
            final transactions =
                ref.read(transactionListNotifierProvider).value ?? [];
            final transaction = transactions.firstWhere(
              (tx) => tx.id == transactionId,
              orElse: () => throw Exception('Transacción no encontrada'),
            );

            // 3. Si lo da, llamar al Notifier de Riverpod que creamos antes
            await ref
